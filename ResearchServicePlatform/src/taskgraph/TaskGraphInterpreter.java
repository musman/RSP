...
public class TaskGraphInterpreter {
	...
	/*
	 * Each template object have its own ID, which can be used to set/get data
	 * variables For global section 0 will be used
	 */
	public Object interpret(TaskGraph first,
			final AbstractQoSRequirement qosRequirement,
			final CompositeService compositeService, final Object... args) {
		...
		while (CT.getTaskType() != ETaskType.RETURN) {
			...
			case PARAM:
				...
				break;
			/*
			 * case LIST_START: list = new LinkedList(); CT = CT.getNext();
			 * break; case LIST_ITEM: Expression exp = (Expression)
			 * CT.getPrev(); list.add(exp.getValue()); CT = CT.getNext(); break;
			 * case LIST_END: ((LIST_END) CT).setValue(list); list = null; CT =
			 * CT.getNext(); break; case DECL:
			 * 
			 * DECL decl = (DECL) CT; Object declType = decl.getType(); Object
			 * type = null;
			 * 
			 * // If is reference
			 * 
			...
			case TERNARY_OP:
				...
				if (ternaryResult) {
					ternaryOp.setValue(((Expression) ternaryOp.getTrue())
							.getValue());
				} else {
					ternaryOp.setValue(((Expression) ternaryOp.getFalse())
							.getValue());
				}
				CT = CT.getNext();
				break;
			...
			case BINARY_OP:
				...
				case LOGIC_OR:
					bExpResult = getBooleanValue(binaryOp.getLeft())
							|| getBooleanValue(binaryOp.getRight());
					break;
				// case OR:
				...
				case BITWISE_INCL_OR:
					bExpResult = getIntValue(binaryOp.getLeft())
							| getIntValue(binaryOp.getRight());
					break;
				...
				case BITWISE_AND:
					...
					break;
				case EQUAL:
					bExpResult = getIntValue(binaryOp.getLeft()) == (getIntValue(binaryOp
							.getRight()));
					break;
				case NOT_EQUAL:
					bExpResult = getIntValue(binaryOp.getLeft()) != (getIntValue(binaryOp
				...
				case GT_EQUAL:
					...
					break;
				case MIN:
					bExpResult = Math.min(getIntValue(binaryOp.getLeft()),
							getIntValue(binaryOp.getRight()));
					break;
				case MAX:
					bExpResult = Math.max(getIntValue(binaryOp.getLeft()),
							getIntValue(binaryOp.getRight()));
					break;
				case LEFT_SHIFT:
					bExpResult = getIntValue(binaryOp.getLeft()) << getIntValue(binaryOp
			...
			case STORE:
				...
				Object rightValue = ((Expression) store.getRightExpression())
						.getValue();
				Object result = null;
				if (varName != null) {
					...
					result = handleAssignOperator(store.getAssignOp(),
					...
					Object prevObject = ((Expression) arrAccess.getPrev())
							.getValue();
					result = handleAssignOperator(store.getAssignOp(),
							arrAccess.getValue(), rightValue);
					Array.set(prevObject, index, result);
				} else if (store.getPrev() instanceof QUALIFIED_ACCESS) {
					QUALIFIED_ACCESS qualifiedAccess = (QUALIFIED_ACCESS) store
							.getPrev();
					String memberId = qualifiedAccess.getVarName();
					Object prevObject = ((Expression) qualifiedAccess.getPrev())
							.getValue();
					result = handleAssignOperator(store.getAssignOp(),
							qualifiedAccess.getValue(), rightValue);
					try {
						prevObject.getClass().getField(memberId)
								.set(prevObject, result);
			...
			case QUALIFIED_ACCESS:
				...
				if (object.getClass().isArray()
						&& qualifiedAccess.getVarName().equals("length")) {
					memberValue = Array.getLength(object);
				} else {
					try {
						memberValue = object.getClass()
								.getDeclaredField(qualifiedAccess.getVarName())
								.get(object);
			...
			case CALL:
				...
				if (call.getServiceName().equalsIgnoreCase("this")) {
					resultInvoke = invokeLocalOperation(
							call.getOperationName(), params);
				} else {
					resultInvoke = invokeServiceOperation(
							call.getServiceName(), call.getOperationName(),
							params);
				}
			...
			case PARALLEL:
				...
				for (int i = 0; i < tasks.size(); i++) {
					...
					futureList[i] = threadpool.submit(new Runnable() {
						...
						public void run() {
							...
							interpreter.interpret(task, qosRequirement,
									compositeService, args);
						}
					});
				}
				...
				break;
			default:
				throw new RuntimeException("Task type is not handled:"
						+ CT.getTaskType().toString());
			}
	...
	private Object handleAssignOperator(EAssignOp op, Object leftValue,
			Object rightValue) {
		Object result;
		switch (op) {
		case MULT_ASSIGN:
		...
		case ASSIGN:
			...
			break;
		default:
			throw new RuntimeException("Assignment operator is not handled:"
					+ op.toString());
		}
		return result;
	}
	 ...
	 */
	public List<ServiceDescription> lookupService(String serviceName,
			String opName) {
		...
		return (List<ServiceDescription>) compositeService.sendRequest(
				ServiceRegistry.NAME, ServiceRegistry.ADDRESS, true, "lookup",
				serviceName, opName);
	}

	protected ServiceDescription applyQoSRequirement(
			AbstractQoSRequirement qosRequirement,
		...
		if (qosRequirement == null) {
			System.err
					.println("QoS requirement is null. To select among multiple services, a QoS requirement must have been provided.");
			System.err.println("Selecting a service randomly...");
			return serviceDescriptions.get(new Random()
					.nextInt(serviceDescriptions.size()));
		}
		return qosRequirement.applyQoSRequirement(serviceDescriptions);
	}

	public Object invokeServiceOperation(String serviceName,
			String operationName, Object[] params) {

		List<ServiceDescription> services = lookupService(serviceName,
				operationName);
		if (services == null || services.size() == 0) {
			throw new RuntimeException(serviceName + "." + operationName
					+ "not found!");
		}
		...
		System.out.println("Operation " + service.getServiceName() + "."
				+ operationName
