...
public abstract class ASTNode extends CommonTree {
	...
	public TaskGraph getFirst() {
	    ...
	    Expression condition = getCondition().get(getCondition().size() - 1);
	...
	public VarRef getLeftExpression() {
	    return (VarRef) getChild(1);
	}
	...
	public TaskGraph getFirst() {
	    ...
	    getRightExpression().setNext(store);

	    store.setPrev(getRightExpression().getLast());
	    store.setAssignOp(((AssignOp) getChild(0)).getAssignmentOp());

	    store.setVarName(getLeftExpression().getId());
	...
	public TaskGraph getFirst() {
	    ...
	    qualifiedAccess.setMemberId(memberId);
	...
	public BooleanLiteral clone() {
	    BooleanLiteral result = new BooleanLiteral(getType(), getLiteralValue() != 0);
	...
	public IntegerLiteral clone() {
	    IntegerLiteral result = new IntegerLiteral(getType(), getLiteralValue());
